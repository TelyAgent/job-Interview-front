...
function SessionNotes({ id, lang, onDirty, onSession, locked, tab }: { id: string; lang: 'zh' | 'en'; onDirty: (dirty: boolean) => void; onSession: (session: MeetingRecord['session']) => void; locked: boolean; tab: string }) {
  ...
  return <>
    {note.record && <div className="record-context">{note.record.session.project.title} · {zh ? `第 ${note.record.session.round} 轮` : `Round ${note.record.session.round}`}<br />{zh ? '私有手工笔记 · 尚未关联 Zoom 场次' : 'Private manual notes · Zoom session not linked'}</div>}
...
export function MeetingRecordPanel({ lang, onDirty, dirty }: { lang: 'zh' | 'en'; onDirty: (dirty: boolean) => void; dirty: boolean }) {
  ...
  const loaded = useCallback((session: MeetingRecord['session']) => { setProjectId(session.projectId); setRound(session.round); }, []);
  useEffect(() => {
    const controller = new AbortController(); setLoading(true); setError('');
    recordApi<{ id: string; title: string }[]>('/meeting-record-projects', { signal: controller.signal })
  ...
  const open = async () => {
    ...
    try {
      const value = await recordApi<MeetingRecord>(`/projects/${projectId}/interview-sessions`, { method: 'POST', body: JSON.stringify({ round }) });
  ...
  return <div className="live-side-panel meeting-record">
    ...
    <div className="live-side-content">
      <div className="record-selector">
        <label>{zh ? '项目' : 'Project'}<select aria-label={zh ? '记录项目' : 'Record project'} value={projectId} disabled={loading || busy} onChange={e => setProjectId(e.target.value)}>
          <option value="">{loading ? (zh ? '加载中' : 'Loading') : (zh ? '选择项目' : 'Select project')}</option>
        ...
        <button disabled={busy || !projectId || !Number.isInteger(round) || round < 1 || round > 100} onClick={() => void open()}>{zh ? '打开记录' : 'Open record'}</button>
      </div>
      {!loading && !projects.length && !error && <p>{zh ? '暂无可访问的真实项目，请先新建面试项目。' : 'No accessible projects. Create an interview project first.'}</p>}
